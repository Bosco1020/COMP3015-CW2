...
void SceneBasic_Uniform::initScene()
{
    ...
    glClearColor(0.05f, 0.65f, 0.8f, 0.5f);
    glEnable(GL_BLEND);
...
void SceneBasic_Uniform::drawScene() {
    ...
    // Transparent Floor
    model = mat4(1.0f);
    model = glm::translate(model, vec3(-3.7f, 0.0f, 1.1f));
    ...
    textProg.setUniform("mossStrength", 1.0f);
    textProg.setUniform("cementStrength", 0.2f);

    model = mat4(1.0f);
    model = glm::scale(model, vec3(40.0f, 1.0f, 1.0f));
    model = glm::translate(model, vec3(-0.0f, 2.0f, -8.0f));
    setMatrices(textProg);
    cube.render();

    textProg.setUniform("cementStrength", 0.0f);

    //Floor Tiles
    model = mat4(1.0f);
    model = glm::translate(model, vec3(14.0f, 0.0f, 1.1f));
    setMatrices(textProg);
    plane.render();
    ...
    blinnProg.setUniform("Material.Ks", 0.9f, 0.9f, 0.9f);
    blinnProg.setUniform("Material.Shininess", 150.0f);
    blinnProg.setUniform("fogStrength", 0.8f);
    ...
    model = glm::translate(model, vec3(4.5f, -1.0f, -18.5f));
    ...
    model = mat4(1.0f);
    model = glm::scale(model, vec3(0.2f));
    model = glm::translate(model, vec3(10.0f, 0.0f, -17.0f));
    model = glm::translate(model, vec3(1.5f * sin(timerB), 0.0f, -1.5f * cos(angle)));
    setMatrices(blinnProg);
    ball.render();

    blinnProg.setUniform("Material.Kd", (color * 0.8f) + (sin(timerA) * vec3(0.1f)));
    blinnProg.setUniform("Material.Ka", ((color * 0.8f) + (sin(timerA) * vec3(0.1f))) * 0.55f);
    model = mat4(1.0f);
    model = glm::scale(model, vec3(0.2f));
    model = glm::translate(model, vec3(8.5f, -0.5f, -18.0f));
    ...
    model = glm::rotate(model, glm::radians(-5.0f), vec3(0.0f, 0.0f, 1.0f));
    model = glm::scale(model, vec3(20.0f, 1.0f, 1.0f));
    model = glm::translate(model, vec3(-0.5f, 0.0f, 3.5f));
    //model = glm::translate(model, vec3(-0.04f, 2.0f, -8.0f));
    setMatrices(blinnProg);
    //cube.render();
}
